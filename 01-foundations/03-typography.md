...
- **โหลดเฉพาะ subset ที่ใช้** อย่ารวมทุก subset เป็นไฟล์เดียว จะกลายเป็น 83 KB แทน 52 KB — ระบบ **self-host แยก 4 ไฟล์พร้อม `unicode-range`** ที่ [`02-tokens/src/fonts.css`](../02-tokens/src/fonts.css) และ `font.spec.ts` ยืนยันในเบราว์เซอร์จริงว่าหน้าไทยดึงมาแค่ `thai` + `latin`
- **`font-display: swap`** ยอมรับการกระพริบครั้งเดียว ดีกว่าข้อความหายบนเน็ตช้า — `check:fonts` บังคับให้ทุก `@font-face` มีบรรทัดนี้
- **ไม่ใช้ Google Fonts CDN จึงไม่ต้อง `preconnect`** — ดู §การส่งมอบฟอนต์ ด้านล่าง
...
## 🚚 การส่งมอบฟอนต์ / Font Delivery

> ★★★ **2026-07-30 — ก่อนวันนี้ระบบไม่เคยโหลด Anuphan เลย**
>
> เอกสารฉบับนี้ parse `GSUB`/`GPOS`/`hmtx`/`cmap` จากไฟล์ฟอนต์จริงมาเขียนไว้ 400 บรรทัด
> แต่ **ไม่มีที่ไหนในโค้ดโหลดไฟล์นั้น** — ไม่มี `@font-face` ไม่มี `<link>` ไม่มี `preconnect`
> `--sme-font-anuphan` เป็นเพียง *ชื่อ* family ที่รอให้ใครมาโหลด · ทุกหน้า ทุกเทส
> ทุก specimen จึง render บน **Noto Sans Thai / ฟอนต์ระบบ** ตลอดอายุของระบบ
>
> **ผลที่ตามมา:** ตัวเลขที่เป็นความกว้าง/การตัดคำที่บันทึกไว้ก่อนวันนี้ทุกค่า
> เป็นค่าของฟอนต์ที่ไม่ได้ใช้ · ฟอนต์ที่โหลดไม่ได้ **ไม่ throw ไม่ error** —
> จึงไม่มีเกตไหนรู้ ตรงตามรูปแบบที่ `CLAUDE.md §5` บันทึกไว้:
> *เอกสารบันทึกความตั้งใจ · โค้ดไม่เคยทำตาม · ไม่มีใครตรวจ*

### วัดเทียบหลังโหลดฟอนต์จริง

Chromium · `400 16px` · `"ขอสินเชื่อธุรกิจ 1,234,567 บาท"`

| ฟอนต์ | ความกว้าง |
|---|---|
| **Anuphan** (ของจริง) | **205.47 px** |
| Noto Sans Thai (ที่ใช้มาโดยไม่รู้ตัว) | 200.90 px |
| ผลต่าง | **Anuphan กว้างกว่า +2.3%** |

2.3% คือเหตุผลที่เทสความกว้างทั้ง 46 ตัวยังผ่านหลังเปลี่ยนมาใช้ฟอนต์จริง — margin
ที่ตั้งไว้กว้างกว่านั้น **แต่ตอนนี้มันผ่านด้วยเหตุผลที่ถูก** · เทสที่มี margin
บางกว่า 2.3% ในอนาคตต้องวัดบนฟอนต์จริงเท่านั้น

### ทำไม self-host ไม่ใช่ CDN

| เหตุผล | รายละเอียด |
|---|---|
| **เทสวัดได้จริง** | e2e ที่วัดความกว้างต้องรันบน Anuphan · ถ้าโหลดจาก CDN แล้วเน็ต CI ช้า เทสจะวัดฟอนต์สำรองแล้ว **เขียว** — ความผิดชนิดเดียวกับที่เรากำลังแก้ |
| **LCP** | `@import` ใน CSS บล็อก render · third-party DNS+TLS บน critical path ทำร้าย 4G ไทย |
| **ความเป็นส่วนตัว** | ไม่ส่ง IP ผู้ใช้ที่กำลังกรอกเอกสารขอสินเชื่อไปให้บุคคลที่สาม |

### เกตที่คุม 2 ชั้น

| เกต | จับอะไร | พิสูจน์ว่า fail ได้ |
|---|---|---|
| `check:fonts` (static) | `@import` หลุด · woff2 หาย · magic ไม่ใช่ `wOF2` · ไม่มี `font-display` / `unicode-range` · แยก subset ไม่ครบ 4 | ✅ ฉีด 4 แบบ แดงทั้ง 4 |
| `tests/e2e/font.spec.ts` (browser) | เบราว์เซอร์ **ดึง woff2 ของเราจริง** · Anuphan อยู่หน้าสุดของ stack ที่ render · หน้าไทยไม่ดึง `latin-ext`/`vietnamese` | ✅ ฉีด 2 แบบ แดงทั้ง 2 |

⚠️ **`document.fonts.check()` เพียงตัวเดียวเชื่อไม่ได้** — มันคืน `true` จากฟอนต์ที่
**ติดตั้งในเครื่อง** ด้วย เทสจะเขียวบนเครื่องนักพัฒนาที่ลง Anuphan ไว้ ขณะที่เว็บจริง
ตกไปใช้ฟอนต์สำรอง · จึงต้องยืนยันจาก **network response** ว่าไฟล์ของเราถูกดึง

⚠️ **`url()` ใน `fonts.css` เป็น relative ต่อไฟล์นั้น** — bundler ของแอปแก้ path ให้เอง
แต่ **Tailwind CLI ไม่แก้** ดังนั้น build ที่ output CSS ไปโฟลเดอร์อื่น (gallery · fixture)
ต้อง `npm run copy:fonts` ตามไปด้วย — ผูกไว้ใน `gallery:build` และ `build:fixture` แล้ว
