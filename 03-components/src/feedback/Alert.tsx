...
/* ═══════════════════════════════════════════════════════════════════════════
   SME.GO · Alert — ข้อความที่อยู่ค้างจนผู้ใช้จัดการ
   ───────────────────────────────────────────────────────────────────────────
   ★★★ **Alert ไม่หายเอง · Toast หายเอง** — นี่คือเส้นแบ่งทั้งหมด

   ระบบนี้ประกาศกฎ "ไม่มีอะไรหายไปตามเวลา" ไว้สามที่ (Tooltip SC 1.4.13 ·
   นับถอยหลัง QR SC 2.2.1 · เหตุผลที่แบน native validation) · Alert คือ
   component ที่ **รักษากฎนั้น** และ Toast คือข้อยกเว้นที่ถูกจำกัดขอบเขตไว้แคบ

   ผลที่ตามมาซึ่งบังคับใช้จริง: **error ใช้ Alert ห้ามใช้ Toast**
   ข้อความที่บอกว่าผู้ใช้ต้องทำอะไรต่อ ห้ามหายไปก่อนที่ผู้ใช้จะอ่านจบ

   ★★★ **`role="alert"` ไม่ใช่ค่าเริ่มต้น และนี่คือการตัดสินใจ**

   `isLive` เป็น `false` โดยค่าเริ่มต้น เพราะ Alert ส่วนใหญ่ถูก render
   ...
   ตั้ง `isLive` เป็น `true` **เฉพาะเมื่อ Alert โผล่มาตอบการกระทำของผู้ใช้**
   ...
   **ไม่ใช่บั๊ก** เพราะตัวเลขนี้อยู่ในย่านเดียวกับ `--color-edge` (1.56) และ
   ...
   Card ตอนพักก็ใช้ค่านั้น · Alert ไม่ใช่ control จึงไม่อยู่ในขอบเขต SC 1.4.11
   ...
   **Alert ที่บอกว่าต้องแก้อะไร ไม่ควรปิดได้เลย**
   ═══════════════════════════════════════════════════════════════════════════ */
...
export interface AlertProps extends VariantProps<typeof alertStyles> {
   ...
   *
   * ⚠️ ตั้ง `true` **เฉพาะ** Alert ที่โผล่มาตอบการกระทำของผู้ใช้
   * Alert ที่ถูก render มาพร้อมหน้าต้องเป็น `false` (ดูหมายเหตุด้านบน)
   *
   ...
   *
   * ⚠️ ห้ามเป็นทางเดียวที่ทำสิ่งนั้นได้ ถ้า Alert ปิดได้
   */
  action?: ReactNode;
   ...
   * ทำให้ปิดได้
   *
   * ⚠️ ผู้เรียกต้องคืน focus หลังปิด · **ห้ามใส่กับ Alert ที่บอกว่าต้องแก้อะไร**
   */
  onDismiss?: () => void;
   ...
   *
   * Alert ไม่ใช่ส่วนหนึ่งของโครงหัวข้อของหน้า · ตั้งค่านี้เฉพาะเมื่อ Alert
  ...
  return (
    <div role={role} className={cn(alertStyles({ tone: resolvedTone }), className)}>
      <div className="flex min-w-0 items-start gap-3">
        ...
        {onDismiss && (
          /* ★ ชื่อปุ่มรวม title — ในหน้าที่มี 3 Alert ปุ่ม "ปิด" 3 อัน
          ...
          <IconButton
            ...
            /* ★ size md = เป้า 36×36 ไม่ใช่ sm ที่ได้ 24×24 พอดีเกณฑ์ —
               Alert ไม่ใช่ component แน่น จึงไม่มีเหตุให้บีบลงถึงขั้นต่ำ */
