...
|---|---|---|
| ข้อผิดพลาดทุกชนิด | [`<Alert tone="danger" isLive>`](./Alert.md) | ข้อความที่บอกว่าต้องทำอะไรต่อ ห้ามหายก่อนอ่านจบ |
| คำเตือนก่อนตัดสินใจ | `<Alert tone="warning">` | ต้องอยู่ค้างระหว่างที่ผู้ใช้คิด |
...
ถ้ามีอะไรให้ทำ **นั่นไม่ใช่ toast** — ใช้ [`<Alert>`](./Alert.md) ที่อยู่ค้าง และทางเลิกทำต้องมีอยู่ที่อื่นบนหน้าด้วยเสมอ
...
"เพิ่มเครื่องคั่วกาแฟ TR-500 ลงตะกร้าแล้ว" ใช้ได้ เพราะตัวเลขบนไอคอนตะกร้าใน [`AppHeader`](../navigation/AppHeader.md) เปลี่ยนไปแล้วด้วย และ [`CartDrawer`](../marketplace/Cart.md) เปิดให้เห็นของจริง
...
**ห้ามมี property `Tone = Error`** — ถ้ามีใน Figma นักพัฒนาจะสร้างขึ้นในโค้ด และเส้นแบ่งกับ `Alert` จะหายไปภายในสปรินต์เดียว
...
| ❌ | ✅ | ทำไม |
|---|---|---|
| toast แจ้ง error | `<Alert tone="danger" isLive>` | ข้อความที่ต้องลงมือทำห้ามหายเอง |
...
| ข้อผิดพลาด (Error) | — | **ห้ามโดยการออกแบบ ไม่ใช่เพราะยังไม่ได้ทำ** — §3 ไม่มี `danger`/`warning` · §7 ห้ามมี property `Tone = Error` ใน Figma · ใช้ [`<Alert tone="danger" isLive>`](./Alert.md) |
