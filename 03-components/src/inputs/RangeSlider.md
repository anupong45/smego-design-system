...
| ช่วงที่ผู้ใช้รู้ค่าแน่นอน | `<NumberField>` สองช่อง | slider เพิ่มความซับซ้อนโดยไม่ช่วย |
| ช่วงวันที่ | `<DateRangePicker>` (Pass 2) | ต้องเป็น พ.ศ. |
| ตัวเลือกไม่ต่อเนื่อง (S/M/L) | `<RadioGroup>` | slider สื่อว่าค่าระหว่างกลางมีอยู่ |
...
2. **ช่องกรอกตัวเลข** ต้นทาง–ปลายทาง พร้อมปุ่มเพิ่ม/ลดของ `NumberField`
