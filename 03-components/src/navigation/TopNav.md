...
⚠️ `<main id="main">` ต้องมีจริง — ลิงก์ข้ามชี้ไปที่นี่ · ใช้ [`<Main>`](../layout/Main.md) แล้วได้ `id` กับ `tabIndex={-1}` ที่จำเป็นครบ ไม่ต้องจำเอง (ก่อน 2026-07-29 ไม่มี component ไหนเป็นเจ้าของ `id` นี้ ลิงก์จึงตายเงียบ ๆ ถ้าแอปลืม)
