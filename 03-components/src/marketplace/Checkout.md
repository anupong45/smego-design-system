...
| ตะกร้าสินค้าแบบแก้ไขได้ | ตาราง + `<NumberInput>` | Summary อ่านอย่างเดียว |
...
| `errorMessage` | `string` | — | `<Banner tone="danger" isLive>` เหนือปุ่ม |
...
| **ผิดพลาด** | `errorMessage` | `<Banner tone="danger" isLive>` **เหนือ** ปุ่ม · ปุ่มกลับมากดได้ |
...
### ★★★ ข้อผิดพลาดเป็น Banner อยู่เหนือปุ่ม

ไม่ใช่ Toast — ข้อความที่บอกว่าต้องทำอะไรต่อห้ามหายไปเอง ([`Banner.md`](../feedback/Banner.md))

อยู่ **เหนือ** ปุ่มเพราะผู้ใช้ที่กำลังมองปุ่มต้องเห็นเหตุผลก่อนกดอีกครั้ง · `isLive` เพราะ Banner นี้โผล่มาตอบการกระทำของผู้ใช้ (SC 4.1.3)
...
async function submit() {
  ...
  try {
    ...
    setError(s.error.network);  /* ★ Banner ไม่ใช่ Toast */
  } finally {
    setSubmitting(false);
  }
}
...
| error จากการยืนยันเป็น toast | `<Banner isLive>` เหนือปุ่ม | ข้อความที่ต้องลงมือทำห้ามหายเอง |
...
| ข้อผิดพลาด (Error) | ✅ | §4 "ผิดพลาด" · เทส **"★★★ ข้อผิดพลาดเป็น `Banner` ที่ประกาศทันที ไม่ใช่ toast"** — การชำระเงินที่ล้มเหลวห้ามแจ้งด้วยของที่หายเอง |
