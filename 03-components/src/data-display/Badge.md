...
Badge เป็นข้อมูลอ่านอย่างเดียว จึงไม่ต้องเป็นเป้า 24×24 และไม่มี focus — ซึ่งเป็นเส้นแบ่งที่ชัดเจนจาก `<Token>`

### ❌ เมื่อไรที่ไม่ควรใช้

| สถานการณ์ | ใช้อะไรแทน | เหตุผล |
|---|---|---|
| กดได้ · ลบได้ · เลือกได้ | `<Token>` | ต้องเป็นเป้า ≥24×24 และมี focus ring |
| ข้อความแจ้งเตือนเต็มบรรทัด | `<Banner>` (Pass 3) | Badge ไม่มี `role="status"` |
...
**นี่คือความแตกต่างที่ทำให้ผู้ใช้แยก Badge ออกจาก Token ได้โดยไม่ต้องอ่านคำอธิบาย** ของที่ไม่ตอบสนองต่อ hover คือของที่กดไม่ได้
...
| คีย์บอร์ด (Keyboard) | — | ป้ายเป็นข้อความ ไม่ใช่ตัวควบคุม — ไม่อยู่ใน tab order โดยตั้งใจ · ถ้าต้องกดได้ให้ใช้ [`<Token>`](./Token.md) |
