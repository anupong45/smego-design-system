...
## 0.3.1 — 2026-07-30 · เมตาดาทาที่ผิดตั้งแต่เวอร์ชันแรกที่ปล่อยจริง

0.3.0 ขึ้น npm สำเร็จเป็นครั้งแรก แล้วสิ่งที่ผู้ใช้เห็นบนหน้า npmjs.com
ก็เปิดเผยว่าเมตาดาทาสามอย่างผิดมาตั้งแต่ต้น — ไม่มีอะไรกระทบโค้ด แต่กระทบ
**สิทธิ์ในการเอาไปใช้** และ **ลิงก์ที่พาไปหาของจริง** ซึ่งเป็นสองอย่างแรกที่คนเปิดหน้าแพ็กเกจมอง

### Fixed

- **`license`** — ไม่เคยมีฟิลด์นี้เลย ⇒ ตามกฎหมายคือสงวนสิทธิ์ทั้งหมด
  แพ็กเกจที่ publish ด้วย `--access public` แต่ประกาศว่าห้ามใช้เป็นสัญญาณที่ขัดกันเอง
  ตอนนี้เป็น **MIT** พร้อมไฟล์ `LICENSE` ที่ root และในโฟลเดอร์แพ็กเกจ
  ⚠️ ต้องมีสำเนาใน **โฟลเดอร์แพ็กเกจ** ด้วย — npm หยิบ `LICENSE` จากรากของ tarball
  เท่านั้น ไฟล์ที่ root ของ monorepo ไม่ถูกส่งไปกับ tarball ของ workspace ใด ๆ
- **`repository.url`** ชี้ `smego-design-system` (ขีดกลาง) แต่รีโปจริงเป็นขีดล่าง
  หน้า repo ของ GitHub redirect ให้ ข้อนี้จึงซ่อนตัวอยู่ได้
- **ลิงก์ gallery ใน CHANGELOG 0.3.0 เป็น 404 จริง** — ประโยคที่โฆษณาว่า
  "gallery deploy อยู่ที่..." พาไปหน้าไม่มีอยู่
  ★ **GitHub Pages ไม่ redirect เวลาเปลี่ยนชื่อรีโป** ต่างจากหน้า repo ที่ redirect
  ⇒ URL ชื่อเก่าตายทันทีและเงียบ · `lint:docs` ไม่จับเพราะไม่ตามลิงก์ภายนอก
- เพิ่ม **`homepage`** ชี้ gallery — เดิมไม่มี หน้า npm จึงไม่มีทางไปดูของจริง

### Fixed — `workflow_dispatch` ของ publish ใช้ไม่ได้มาตลอด

ประกาศไว้เป็น `workflow_dispatch:` เปล่า ๆ ⇒ dispatch รันบน **branch** ทำให้
`github.ref_name` เป็น `master` · เกตเทียบเวอร์ชันคำนวณ `TAG` ได้เป็น `master`
แล้วตายทันทีทั้งสอง job — **ปุ่มนั้นเป็นของประดับ** ตอนนี้รับ input `tag`
และทุกจุดที่เคยอ่าน `github.ref_name` อ่าน `inputs.tag || github.ref_name` แทน
รวม `if:` ของ job และ `ref:` ของ checkout

## 0.3.0 — 2026-07-30 · เวอร์ชันแรกที่ส่งมอบได้จริง
...
— ตอนนั้นระบบยังไม่เคยมี consumer นอกรีโปเลย (`git remote` ว่าง · `git tag` ว่าง)

เวอร์ชันนี้คือเวอร์ชันแรกที่ **ส่งมอบได้จริง**: มี `build` → `dist/` ที่ publish ได้ ·
ขอบเขต client/server ที่รอดมาถึง `dist` · ฟอนต์ที่ถูกโหลดจริง · โหมดมืดที่เปิดใช้ได้ ·
และ gallery ที่ deploy อยู่ที่ <https://anupong45.github.io/smego_design_system/>
(ยืนยันบนเว็บจริง: ดึง woff2 2 subset · Anuphan 2 face loaded · `scrollWidth 320`
ที่ viewport 320 ⇒ SC 1.4.10 ผ่าน · specimen 59 ตัว)
...
✅ **ช่อง "หัก API แล้วลืมขยับเวอร์ชัน" ปิดแล้ว** — `publish.yml` มีขั้นที่ล้มถ้า
tag ไม่ตรงกับ `version` ใน `package.json` · ช่องนี้เคยทำให้ 0.2.0 ค้างอยู่ที่ `0.1.0`
หลายวันโดยไม่มีเกตไหนรู้
