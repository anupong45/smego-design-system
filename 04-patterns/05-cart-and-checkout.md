...
| เพิ่มลงตะกร้าไม่สำเร็จ | `<Banner tone="danger" isLive>` ใน `BuyBox` | ต้องอ่านจบก่อนหาย |
| ตะกร้ามีหลายร้าน | `<Banner tone="info">` ใน `CartList` | เป็นเงื่อนไขที่ต้องรู้ก่อนกดชำระ |
| ยืนยันคำสั่งซื้อไม่สำเร็จ | `<Banner isLive>` ใน `CheckoutSummary` | เดียวกัน |
| ผู้ขายไม่จด VAT | `<Banner tone="warning">` | ต้นทุนจริงต่าง 7% |

**ไม่มีเหตุการณ์ใดในเส้นทางนี้ที่ใช้ toast แจ้ง error** — เส้นแบ่งอยู่ใน [`Banner.md`](../03-components/src/feedback/Banner.md)
...
3. **`<main id="main">` ต้องมี** เพราะลิงก์ข้ามใน `TopNav` ชี้มาที่นี่
