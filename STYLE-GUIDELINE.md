...
## 3 · สิ่งเดียวที่ควรรับมาจาก Astryx — และยังไม่มี

**agent-readable docs ผ่าน CLI และ MCP**

จุดขายหลักของ Astryx คือ *คนกับ AI สร้างงานจากเอกสารชุดเดียวกันด้วยเครื่องมือชุดเดียวกัน* — CLI พิมพ์เอกสาร component ออกมาแบบเครื่องอ่านได้ และเปิดผ่าน MCP ได้

**SME.GO ยังไม่มี** เรามีแค่ `.md` ต่อ component ซึ่งคนอ่านได้แต่ agent ต้องเดาโครงสร้าง

**ต้นทุนต่ำเพราะของเรามีเงื่อนไขพร้อมอยู่แล้ว**

- `.md` ทั้ง 60 ไฟล์ใช้หัวข้อ 9 ข้อเรียงลำดับเดียวกัน → parse ได้
- [`tokens.json`](02-tokens/tokens.json) เป็น DTCG เครื่องอ่านได้อยู่แล้ว
- [`validate-tokens.js`](02-tokens/validate-tokens.js) และ [`gen-figma.js`](02-tokens/gen-figma.js) เป็น CLI อยู่แล้ว

**ข้อเสนอ:** เพิ่ม `smego docs <Component>` และ `smego tokens <query>` ในชั้น 06 Documentation แล้วเปิดผ่าน MCP — ไม่ใช่งานใหญ่ และทำให้เอกสารที่เขียนไว้ถูกใช้จริงมากขึ้น
...
| # | ช่องว่าง | สถานะ |
|---|---|---|
| 1 | **React Aria ไม่มี `th-TH`** ใน 34 locale — ผู้ใช้ TalkBack ไทยได้ยินอังกฤษเมื่อ RAC ประกาศเอง | คำแปล ~30 ข้อความพร้อมที่ [`strings-rac.th.ts`](03-components/src/lib/strings-rac.th.ts) · **วิธีต่อเข้า runtime ยังไม่ตัดสิน** |
